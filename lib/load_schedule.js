...
module.exports = app => {
  let dirs = app.loader.getLoadUnits().map(unit => path.join(unit.path, 'app/schedule'));
  const directory = app.config.schedule && app.config.schedule.directory;
  if (Array.isArray(directory)) {
    dirs = dirs.concat(directory);
  }
